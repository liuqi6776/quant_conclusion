...
| [hot_and_chip_enhanced_report.md](./hot_and_chip_enhanced_report.md) | **【月频全要素终极大协同】连板排雷 + 同花顺热股散户逆向排雷 + 筹码月度诊断实证研报**：2759交易日同花顺热股 Rank IC -0.0738 (散户接盘见顶排雷护盾) + 筹码单边排深套诊断 + 债券黄金多资产协同 | 🏆 **月频高收益巅峰定论**（年化 **12.00%**、夏普 **1.02**、最大回撤仅 **-13.43%**、累计总收益 **+51.62%**，跑赢中证1000达 **+34.39%**） |
| [peg_granville_streaks_report.md](./peg_granville_streaks_report.md) | **【新维度因子实证】市值/PEG + 葛式八法 + 连涨连跌数全景研报**：78期月度截面 Rank IC 检验 + 估值稀释动量归因 + 生产级多方案消融对比 | 🎯 **增量定论**（基准保持最高年化 **12.00%** 与夏普 **1.02**；PEG(t=1.01)与微盘因子存在滞后与黑天鹅风险；估值强行并入树模型稀释动量 Alpha；当前基准处于最优帕累托前沿） |
| [board_segmentation_report.md](./board_segmentation_report.md) | **【板块实证与ST排雷】A股分板块（主板/创业板/科创板/北证）与排除ST/退市实证研报**：时点排除 ST/退市标的 + 4大板块 (主板±10%/创业板±20%/科创板±20%/北交所±30%) 独立生产级对照 | 🏆 **板块选型与排雷定论**（排除ST/退市显著提升策略CAGR至 **13.47%**、夏普 **0.99**、回撤降至 **-10.93%**；创业板为核心Alpha动量引擎 CAGR 12.15%，北交所高弹性低容量 CAGR 10.35%，主板为容量底仓 CAGR 7.66%，科创板阶段周期受制 CAGR 5.66%；全市场跨板块动态优选为最优方案） |
...
| [cs_relational_transformer_report.md](./cs_relational_transformer_report.md) | **【高性价比特征工程与截面 Transformer 路径三】**：格拉姆-施密特正交化紧凑特征集 (14维) + 截面分层关系注意力 CS-Transformer (行业内自注意力+跨行业全局注意力+PearsonRankLoss) + 跨范式正交融合 | 🏆 **深度学习与特征工程定论**（正交特征工程将 GBDT 夏普由 0.20 提升至 0.27；CS-Transformer 样本外 IC 达 +0.0575、IC 胜率 80.6%；跨范式正交融合 ★ ENS-Hybrid-CS 实现年化 6.68%、夏普 0.45、回撤 -23.31%，模型预测相关度仅 0.097，夏普比率翻倍提升 125%） |
| [ens_hybrid_cs_sentiment_timing_report.md](./ens_hybrid_cs_sentiment_timing_report.md) | **【短线情绪周期择时与 ENS-Hybrid-CS 选股引擎深度融合实证】**：五大短线情绪指标连续 SCS + 方案 1C 成本感知平滑 + 已有持仓篮子等比例缩放 `scale_stock_exposure` + 生产级微观单现金池真实账本 v2.2 全策略公平消融比武 | 🏆 **当前终极生产定论 (Ultimate Production Standard)**（在真实微观账本约束下：纯选股 Alpha 遭遇小微盘挤兑年化仅 -0.87%/夏普 0.03/回撤 -43.64%；接入连续线性 SCS 择时避险后年化跃升至 10.35%/夏普 0.67/回撤 -11.89%；★ 终极多要素协同方案叠加逆波动率预算与国债/黄金防御停泊，斩获年化 **10.67%**、夏普 **0.83** (较指数基准提升近 4 倍)、年化波动率大幅压缩 60% 至 **10.54%**、最大回撤收敛至 **-12.29%**、卡玛比率达 **0.87**、总收益 **+45.18%**，全部策略通过严密数学自洽连乘检验 0.000000% 零误差） |
| [audit_remediation_round6_report.md](./audit_remediation_round6_report.md) | **【第六轮独立审计整改与基准重建全景报告】**：正式废止旧版未经审计指标 + 100% 纯现金防守腿构建纯净 ETF+SCS 基准 (`etf_scs_clean_v1` B0 年化 10.84%/夏普 0.74/回撤 -9.97%；B1 年化 10.31%/夏普 0.75/回撤 -10.44%) + CS-Transformer 主动增量配对评测 (`cs_transformer_scs_clean_v1` B1 年化 16.68%/夏普 1.26/回撤 -8.89%，增量 Alpha +6.37%/夏普 +0.51/IR 0.77) + A1 暴雷过滤独立消融 (年化 17.00%/夏普 1.28/解耦作为风险开关) + P0-10 CH4 收益区间精确对齐与 Newey-West HAC 稳健检验 (基准 MKT Beta 1.274, R^2 0.9808；策略纯 Alpha 12.83%~12.96%, t=2.42~2.43, p=0.015) + 全套 13 项标准制品导出 | ⚖️ **第六轮审计整改验收基线 (Round 6 Audit Verified Baseline)** |

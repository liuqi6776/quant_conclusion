...
### 收益与行业层增量：成长制造白名单过滤实验（2026-08-18 审计核验）

| 实验变体 | CAGR | Sharpe | 日频 MaxDD | **月频 MaxDD** | Calmar | 平均持股 | 单行业最大占比 | 判定 |
|---|---|---|---|---|---|---|---|---|
| **基线 (全市场 Top40 无过滤)** | 11.80% | 0.77 | -25.48% | **-19.02%** | 0.46 | 40.0 只 | 9.8% | ✅ 冻结基准 |
| **变体1: 选股后过滤 GM_FULL** | 12.27% | 0.80 | -25.10% | **-18.65%** | 0.49 | 30.8 只 | 12.4% | 🔬 探索线索 |
| **变体2: 选股后过滤 GM_CORE** | 14.14% | 0.87 | -24.25% | **-17.08%** | 0.58 | 13.6 只 | 28.0% | ⚠️ 伪增量(集中度虚高) |
| **变体3: 选股前过滤 GM_CORE (池内限额)** | 13.32% | 0.82 | -24.60% | **-18.19%** | 0.54 | 40.0 只 | 10.0% | 🔬 探索线索 |

> **方法学教训与局限核验**：
> 1. **静态行业分类后见之明偏差**：`ind_map` 是 2026 静态分类回溯到 2019 年选股，用今天的成长股名单挑过去 6 年的历史，对 2020 年 (+26.8%) 和 2023 年 (+12.6%) 产生显著正向虚高。
> 2. **后置过滤变相架空行业上限**：`GM_CORE` 选股后过滤使持仓数量从 40 只骤降至 13.6 只，单行业平均占比升至 28.0%（高度集中于汽车零部件与半导体），退化为赛道集中押注，破坏了截面多因子的分散化保护。
> 3. **月频回撤无实质改善**：月频最大回撤维持在 -17%~-19%，且在 2024 年冲击期前置过滤收益 (+9.0%) 显著低于基线 (+12.6%)。
> 4. **裁定**：**不作为超越基线的生产结论采信**，维持全市场分散化 Top40 为基准。

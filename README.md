...
> ⚠️ 现状（2026-09 审查更新）：**当前仓库不存在 ✅ 可用结论**。各品类核心瓶颈如下：场外基金（FUND）模块虽已建立 50+ 项全自动回归测试、SHA-256 账本哈希锁定与无前视走步引擎，但整体样本外范围仍定性为 `component_only`（基础 7 资产标的池仍含事后选品痕迹）；股票（STOCK）模块则存在数据/代码不可独立复现、缺少自动测试等问题。所有正向结论统一标为 ⚠️ 候选，请参照文档头部六维正交标签判断证据强度。
...
- [场外基金 FUND](./FUND/README.md)
  - [场外基金多资产稳健定投策略：2015-2026 审计重构与全周期实证（quant_system_v2）](./FUND/otc_fund_dca_2015_2026.md) — ⚠️ 候选（11.6 年定投 140 万，期末 313.2 万~319.8 万，XIRR 13.2%~13.5%，TWR 最大回撤 -11.2%~-12.4%；底仓+定投投入 240 万期末 695.6 万~711.0 万；显式分红再投资驱动与宽基指数对照组）
  - [场外基金稳健组合：低相关分散 + 留一归因 + VolTarget 7%（quant_system_v2）](./FUND/otc_fund_stable_portfolio.md) — ⚠️ 候选（7资产相关系数均值 0.119，留一剔除归因测算，VolTarget 7% 动态风控与闲置现金计息）
  - [基金研究教训反思与实证结论分级：4433/分红/幸存者偏差/血缘清单](./FUND/otc_fund_lessons.md) — ⚠️ 候选（含 ❌ 2大证伪子项与 ⚠️ 3大配置规则，新增分红再投资基线、样本内幸存者偏差警示与清单血缘闭环）
